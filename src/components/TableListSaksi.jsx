...
export default function TableListSaksi({token, setIsOpenModalSaksi, setNameKoordinator, setHpKoordinator}) {
  ...
  const tableEdit = (kecamatan_name, kelurahan_name, tps_name, user_id, name_koordinator, hp_koordinator, code) => {
    return (
      <TableCell
        component="th"
        scope="row"
        ...
        <TableBody>
          {(rowsPerPage > 0
            ...
            <TableRow key={key}>
              ...
              {tableEdit(row.kecamatan_name, row.kelurahan_name, row.tps_name, row.user_id, row.name_koordinator, row.hp_koordinator, row.code)}
            </TableRow>
          ))}
          {emptyRows > 0 && (
